...
save = True
# path_savefolder = abs_path + f"Data/PGM/Displacement-Force/exp2/Length_{PGM_length}/PGM_number_{PGM_number}/"
path_savefolder = abs_path + f"Data/PGM/Displacement-Force/exp2/test/"
...
def onPositionChange(self, position):
    if is_experimenting:
        ...
        print(f"Displacement: {current_position_mm:.2f} [mm], Force: {force:.2f} [N]")
...
class Load_data():
    ...
    def main(self, length, num):
        ...
        return OFFSET, GAIN, atomosphere, target_position
...
async def main():
    global start_time, results, phidget
    ...
    try:  # オフセットの読み込み
        load = Load_data()
        OFFSET, GAIN, atmospheric_pressure, target_position = load.main(PGM_length, PGM_number)
    except Exception as e:
    ...
    try:  #PGMの接続
        is_connected = False, time.sleep(3)
    ...
    ## EXPERIMENT ##
    print("\n===== Start experiment =====")
    print(f"===== Target position: {target_position} [mm] ====="), time.sleep(1)
    ...
    try:
        ...
        phidget.set_target_position(0, velocity, acceleration)
        check_target_position()
    ...
    try:
        ...
        if save:
            try:
                ...
                print(f"Some error occured durinig save: {e}")
